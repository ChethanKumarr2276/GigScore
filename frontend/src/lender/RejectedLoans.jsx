...
export const RejectedLoans = () => {
  ...
  useEffect(() => {
    ...
    async function fetchApplicants() {
      ...
      try {
        ...
        console.error('[RejectedLoans] Failed to fetch /applicants:', error);
        if (isMounted) {
  ...
  const lenderNavItems = [
    ...
    { label: 'Rejected Loans', icon: XCircle, path: '/lender/rejected', badge: `${rejectedLoans.length}` },
  ...
  const totalPages = Math.max(1, Math.ceil(rejectedLoans.length / PAGE_SIZE));

  const paginated = useMemo(() => {
    const start = (currentPage - 1) * PAGE_SIZE;
    return rejectedLoans.slice(start, start + PAGE_SIZE);
  }, [rejectedLoans, currentPage]);
        ...
        <main className="p-4 sm:p-6 lg:p-8 max-w-7xl w-full mx-auto space-y-6">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
              Applicants whose loan decision was recorded as Rejected.
            </p>
          </div>
          ...
          <Card className="p-4 max-w-xs">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Total Rejected</p>
          ...
          <Card className="overflow-hidden border border-slate-200 dark:border-slate-800">
                  ...
                  {isLoading ? (
                    ...
                    paginated.map((app) => (
                      <tr key={app.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                        ...
                        <td className="px-4 py-4">
                          <div className="font-extrabold text-red-500 dark:text-red-400">
                            {app.requestedAmount}
                          </div>
                        </td>
            ...
            {!isLoading && rejectedLoans.length > 0 && (
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-4 py-3.5 border-t border-slate-200 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-900/40">
                <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">
                  ...
                  {'–'}
                  <span className="font-bold text-slate-700 dark:text-slate-200">
                    {Math.min(currentPage * PAGE_SIZE, rejectedLoans.length)}
                  </span>{' '}
                  of{' '}
                  <span className="font-bold text-slate-700 dark:text-slate-200">{rejectedLoans.length}</span>{' '}
                  rejected loans
                </p>
